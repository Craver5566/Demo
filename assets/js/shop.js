...
let shop = {
    ...
    'renderElements': function () {
        for (let i = 0; i < this.allProducts.length; i++) {
            this.productsContainer.innerHTML += `<div class="product" id="product-` + (i + 1) + `">
                ...
                <input type="text" name="" id="text" class="text" placeholder="輸入數量">
            </div>`
        }
    },
    ...
    'fetchProducts': function () {
        ...
        httpRequest.addEventListener('readystatechange', function(){
            if ( httpRequest.readyState === 4 && httpRequest.status === 200) {
               this.allProducts = JSON.parse(httpRequest.responseText);
            }
        }.bind(this));
        httpRequest.open("GET", this.urls.getProducts, false); // open() 還能接受第三個參數, 是用以表示這個 request 是否為 asynchronous (非同步) 的布林值, 預設為 true
        httpRequest.send();
    ...
    'submit': function () {
        ...
        let data = {
            'token': 'c2e0c471304894aea5c1bf11c71a993122439e7fafb654c79f8a7ed711485449',
